...
@Component({
  selector: 'validation-five',
  templateUrl: './validation-five.component.html',
  styleUrls: ['./validation-five.component.scss']
})
export class ValidationFiveComponent {
  ...
  getFilesFromClient() {
    ...
    this.clientService.getAllClientFiles(this.idClient).subscribe((res) => {
        console.log(res);
  ...
  public onAccepted(clientId: number, index: number) {
    ...
    this.clientService.validateFive(clientId, { status: true }).subscribe((res) => {
      console.log(res);
      if (res.status) {
  ...
  public onRejected(clientId: number, index: number) {
    console.log(clientId);
    this.clientService.validateFive(clientId, { status: false }).subscribe((res) => {
      console.log(res);
      if (res.status) {
        this.clientsArray.splice(index, 1);
        this.selectedIndex = -1;        
