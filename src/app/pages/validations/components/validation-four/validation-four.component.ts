...
@Component({
  selector: 'validation-four',
  templateUrl: './validation-four.html',
  styleUrls: ['./validation-four.scss']
})
export class ValidationFourComponent {
  ...
  public onAccepted(clientId: number, index: number) {
    ...
    this.clientService.validateFour(clientId, { status: true }).subscribe((res) => {
      console.log(res);
      if (res.status) {
  ...
  public onRejected(clientId: number, index: number) {
    console.log(clientId);
    this.clientService.validateFour(clientId, { status: false }).subscribe((res) => {
      console.log(res);
      if (res.status) {
        this.clientsArray.splice(index, 1);
        this.selectedIndex = -1;        
